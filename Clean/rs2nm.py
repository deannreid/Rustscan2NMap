...
def main():
    try:
        ...
        # Set the default output directory to the current directory if not provided
        save_location = os.getcwd() if len(sys.argv) < 3 else sys.argv[2]

        print(Fore.YELLOW + "Just running a quick wee rustscan test\nYou selected: {}".format(Fore.RED + The_Bad_Guy + Style.RESET_ALL))
        ...
        while True:
            ...
            if confirmation.lower() == 'k':
                print(Fore.RED + "\n\nHurting my feelings..." + Style.RESET_ALL)
                sys.exit(0)
